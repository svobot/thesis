...
\begin{center}
  \janus{(λ\textbf{x} \textbf{y}. y) : (\quant0 \textbf{x} : U) -> (\quant1\
  \textbf{y} : x) -> x} \\ \vspace{0.6em}
\janus{(λ\_ \textbf{y}. y) : (\quant0 \textbf{x} : U) -> (\quant1 \_ : x) -> x}
\end{center}
...
\begin{center}
  \janus{λ\textbf{x} \textbf{y} \textbf{z}. x} \hspace{3em} instead of
  \hspace{3em} \janus{λ\textbf{x}. λ\textbf{y}. λ\textbf{z}. x}
\end{center}
...
equivalent:
\begin{center}
  \janus{(\vmany{} \_ : $S$) → \vmpairt{\vmany}{\_}{$T$}{$S$}} \hspace{6em}
...
\begin{center}
  ...
  (\quant{$\pi_n$} $x_n$ : $S_n$)}
\end{center}
for $n \geq 1$; where \quant{$\pi_i$} is a~semiring element, $x_i$ a~variable,
...
\begin{center}
  \janus{assume (\quant0 $x$ : U) (\quant1 $y$ : (\quant1 \_ : $x$) -> $x$)}
\end{center}
...
\begin{center}
\janus{\vmany{} (λ\textbf{x} \textbf{y} \textbf{z}. x) : ∀ (\vmany{} \textbf{x}\
 : \vuniv) (\vmany{} \textbf{y} : \vuniv) (\vmany{} \textbf{z} : \vuniv) .\
 \vuniv}
\end{center}
