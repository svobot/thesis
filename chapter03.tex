...
that both sets of rules assign a~type to the~same set of terms. The~first rule,
\ir{Id}, is an~axiom and the~rest are the~logical rules:
\begin{mathpar}
  \inferrule*[right=Id]
  { }
  {x \is{} S \vdash x \is{} S} \\
...
\begin{enumerate}
  ...
  \item if $M^{S \times T}$ is a~typed \lt of type $S \times T$, then
    the~following are typed \lts of types $S$ and $T$, respectively:
    \begin{mathpar}
      (\fst M^{S \times T})^S \and
      (\snd \: M^{S \times T})^T.
    \end{mathpar}
\end{enumerate}
...
indicate whether Contraction or Weakening may be used.
...
\begin{definition}
  ...
  \begin{enumerate}
    \item all typed variables $x^T$ are terms of type $T$;
    \item if $M^{S \multimap T}$ and $N^S$ are terms of types
      $S \multimap T$ and $S$ respectively, then the~following is a~term of type
      $T$:
      \[
        (M^{S \multimap T} \: N^S)^T;
      \]
    \item if $x^S$ is a~typed variable and $M^T$ is a~term of type $T$, then
      the~following is a~term of type $S \multimap T$:
      \[
        (\lam {x^S} {M^T})^{S \multimap T};
      \]
    \item if $M^S$ and $N^T$ are terms of types $S$ and $T$ respectively, then
      the~following is a~term of type $S \otimes T$:
      \[
        \mpair {M^S} {N^T} ^{S \otimes T};
      \]
    \item if $x^S$ and $y^T$ are typed variables and $M^{S \otimes T}$ and $N^U$
      are terms of type $S \otimes T$ and $U$ respectively, then the~following
      is a~term of type $U$:
      \[
        (\textrm{let} \: \mpair {x^S} {y^T} = M^{S \otimes T} \: \textrm{in} \:
        N^U)^U;
      \]
    \item if $M^S$ and $N^T$ are terms of types $S$ and $T$ respectively, then
      the~following is a~term of type $S \with T$:
      \[
        \apair {M^S} {N^T} ^{S \with T};
      \]
    \item if $M^{S \with T}$ is a~term of type $S \with T$, then the~following
      are terms of type $S$ and $T$ respectively:
      \begin{align*}
        (\fst M^{S \with T})^S&  &  &(\snd M^{S \with T})^T;
      \end{align*}
    \item if $M^S$ is a~term of type $S$, then the~following is a~term of type
      $\oc S$:
      \[
        (\oc M)^{\oc S}.
      \]
...
\begin{mathpar}
  ...
  {\Gamma \mid \Delta \vdash \lam {x^S} M \is{} S \multimap T}
...
\begin{mathpar}
  ...
  }
  {
    ...
    \vdash \textrm{let} \: \mpair x y = M \: \textrm{in} \: N \is{} U
  }
\end{mathpar}
...
Now consider the~implications of having a~pair as a~hypothesis. Take $P$ to be
the~proposition ``I am happy.'' Then the~judgments
\begin{mathpar}
  \diamond \mid x \is{} T \otimes U \vdash M \is{} P \and
  \diamond \mid x \is{} T \with U \vdash M \is{} P \and
  \diamond \mid x \is{} T \oplus U \vdash M \is{} P
\end{mathpar}
...
\begin{mathpar}
  ...
  {
    \Gamma \mid \Delta_1, \Delta_2
    \vdash \textrm{let} \: \oc x = M \: \textrm{in} \: N \is{} T
  }
\end{mathpar}
...
\begin{mathpar}
  \inferrule*[Right=$\multimap$-E]
  {
    \inferrule*[right=Weak]
    {
      ...
      {
        \Gamma \mid \Delta
        \vdash \lam {x^{\oc S}} M \is{} \oc S \multimap T
      }
    }
    {
  ...
  }
  {
    ...
    \vdash \textrm{let} \: \oc x = x \: \textrm{in} \: N \is{} T
  }
\end{mathpar}
