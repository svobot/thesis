...
each β-reduce to one of the~elements of the~pair. We can modify the~definition
...
a~unit type, we again modify the~\autoref{def:stlc}, this time by simply
removing the~term
\[
  \letu M N,
\]
...
The~introduction rule is also identical to the~old definition, but now there is
no elimination rule:
\begin{mathpar}
  \inferrule*[right=\1-I$'$]
  { }
...
The~following establishes the~equivalence on the~terms: every occurrence of
the~unit eliminator can be expressed using only the~\emph{in} part of the~term:
\[
  \letu M N \defeq N
\]
