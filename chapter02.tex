...
For example, the~derivation tree of the~judgment that given that $A$ implies $B$
and given $A$, one may deduce both $A$ and $B$:
\begin{mathpar}
  \inferrule*
  {
    \inferrule*
...
Throughout the~text we use the~letters $S$, $T$, and $U$ to denote arbitrary
types.
...
\begin{definition}[Simply typed \lts]\label{def:stlc}
  ...
  \begin{mathpar}
    \lam {x \is{} S} M \and M \: N \and \mpair M N \\
    \letp {x \is{} S} {y \is{} T} M N \and \letu M N
  \end{mathpar}
\end{definition}
...
\begin{mathpar}
  ...
  {
    \Gamma \vdash \letp {x \is{} S} {y \is{} T} M N \is{} U
  }
\end{mathpar}
...
\begin{definition}\label{def:pseudo-term}
  ...
  \begin{mathpar}
    \dep x M N \and \lam {x \is{} M} N \and M \: N \and \mpair M N \\
    (x \is{} M) \times N \and \mpair M N \and \letp{x \is{} S}{y \is{} T} M N \\
    \letu M N
  \end{mathpar}
\end{definition}

Although the~\autoref{def:pseudo-term} mentions it separately, the~unit type \1
...
derive type formation, hence the~\ir{-F} suffix, for the~dependent function
...
\begin{mathpar}
  ...
  \inferrule*[right=$\times$-E]
  {
    ...
    \Gamma, x \is{} S, y \is{} T \vdash N \is{} [(x, y)/z]U
  }
  {
    \Gamma \vdash \letp {x \is{} S} {y \is{} T} M N \is{} [M/z]U
  }
\end{mathpar}
...
\begin{mathpar}
  ...
  \inferrule*[right=\1-E]
  {
    ...
    \Gamma \vdash M \is{} \1 \\
    \Gamma \vdash N \is{} [\munit/x]U
  }
  {
    \Gamma \vdash \letu M N \is{} [M/x]U
  }
\end{mathpar}

Now we can look at an~example of quantification over \univ that we mention at
