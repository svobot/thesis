...
\begin{definition}[Simply typed \lts]\label{def:stlc}
  ...
  \begin{mathpar}
    ...
    \letpair{S \times T} x y M N \and \letu M N
  \end{mathpar}
\end{definition}
...
\begin{definition}\label{def:pseudo-term}
  ...
  pseudo-terms; and if $x$ and $y$ are variables and $M, N, S, T$ and $U$ are
  pseudo-terms, then the~following are pseudo-terms:
  \begin{mathpar}
    \dep x M N \and \lam {x \is{} M} N \and M \: N \\
    (x \is{} M) \times N \and \mpair M N \and
    \letpair{(x \is{} S) \times T}[z] x y M [U] N \\
    \letu M N
  \end{mathpar}
\end{definition}

Although the~\autoref{def:pseudo-term} mentions it separately, the~unit type \1
is an~atomic type. Let us consider what other atomic types can a~given system
have. First, there are all the~familiar types, like the~type of integers
