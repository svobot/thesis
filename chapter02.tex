...
\begin{mathpar}
  ...
  {Q}.
\end{mathpar}
It says that if ``$P$ implies $Q$,'' and ``$P$,'' then ``$Q$.'' Note that some
rules have a~name, which is attached to the~right side of the~horizontal line.
In our example the~name is \ir{Mp}.
...
\begin{mathpar}
  \inferrule*[right=Mp$'$]
  {\Gamma_1 \vdash P \to Q \\ \Gamma_2 \vdash P}
...
\begin{definition}
  ...
  \emph{atomic types}; then we define \emph{types} as follows:
  \begin{enumerate}
    \item every atomic type is a type;
    \item if $\sigma$ and $\tau$ are types, then $(\sigma \to \tau)$ is a~type,
      called a~\emph{function type};
    \item if $\sigma$ and $\tau$ are types, then $(\sigma \times \tau)$ is
      a~type, called a~\emph{pair type};
  \end{enumerate}
\end{definition}
...
functions from $\sigma$ to $\tau$, or just some subset. Some examples of types
are:
\begin{align*}
...
of the~type. This can be expressed with the~ability to universally quantify
the~types. With universal quantification, we are able to say that the~type of
the~identity function should, in effect, be `for all types $\tau$, $\tau \to
\tau$'.

In this chapter we will replace the~function and pair types with their dependent
variants. The~dependent analogue of the~type $\sigma \to \tau$ is
the~\emph{dependent function type}, in the~academic literature usually written
as ${(\Pi x \is{} \sigma \, . \: \tau(x))}$, but we will write:
\[
  \dep x \sigma {\tau(x)}.
\]
It can be informally read as ``for all $x$ in $\sigma$, $\tau(x)$''. Here,
$\sigma$ is a~type, but $\tau(x)$ is a~function whose argument is of type
$\sigma$ and it produces value that is a~type. Hence, a~term of type $\dep x
\sigma {\tau(x)}$ represents function that for an~argument $N$ of type $\sigma$
produces a~value of type $\tau(N)$.

Thus, in the~special case that $\tau(x)$ is a~constant function whose value is
always the~type $\rho$, $\dep x \sigma {\tau(x)}$ is the~type $\sigma
\to \rho$.
...
\]
The~type of the~second element of a~pair may be determined by the~value of its
first element.
...
The~two distinct atomic types are required for the~type system to by consistent
when used as a~system of logic~\todo{[CITE?]}. This is not necessary for our
needs however, so we will simplify by having a~single atomic type and indeed
formulate an~inconsistent type system~\todo{[CITE?]}.
This means that every type contains some value, or is \emph{inhabited}.

Thus, we define only one atomic type \univ, analogous to Barendregt's $\star$,
with a~rule
\begin{mathpar}
  \inferrule*
  { }
...
The~atomic type \univ represents the~type of types. It is called
the~\emph{universe}.
