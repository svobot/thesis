...
\begin{definition}
  ...
  \: \top$, and $\univ$ are pseudo-terms; and if $M, N, S, T$ and $U$ are
  pseudo-terms, $\pi$ and$ \rho$ are semiring elements, and $x$ and $y$ are
  variables, then the~following are pseudo-terms:
  \begin{mathpar}
    \lam {x \is\pi M} N \and M \: N \and \depq x \pi M N \\
    \mpair M N \and \letpair {(x \is\pi S) \otimes T} [z] x y M [U] N \and
      (x \is\pi M) \otimes N \\
