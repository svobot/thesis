...
a~context with no resources. The~Universe formation rule, \ir{Univ}, is only
...
elimination rules combine the~type parametrisation of \nameref{sec:dtlc} with
...
\begin{enumerate}
  \item in the~order $1 \leq \omega$, quantity $\omega$ means that a~resource
    is computationally relevant;
  \item order $0 \leq 1 \leq \omega$ creates affine typing system;
  \item in the~order $0, 1 \leq \omega$ (but not $0 \leq 1$), quantity $\omega$
    represents an~intuitionistic resource. This makes $\depq x \omega S T$
    behave like an~intuitionistic function while $\depq x 1 S T$ behaves like
    a~linear one. This is the~order that is used in our interpreter.
\end{enumerate}
